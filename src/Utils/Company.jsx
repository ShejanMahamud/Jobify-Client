...
const navigate = useNavigate();
  ...
  return (
    ...
    <img src={company?.company_logo || 'https://i.ibb.co/jMNrMnz/enterprise.png'} alt="" className='h-16 w-16 rounded-lg object-cover p-3 bg-white shadow-lg'/>
...
<span className="text-[#636A80] text-sm">{company?.location}</span>
              </div>
...
<span className="text-[#636A80] text-sm">{company?.open_jobs} - Open Job</span>
    ...
    <button onClick={()=>navigate(`/company/${company?._id}`)} className="bg-[#E7F0FA] px-4 py-3 rounded-md text-primary font-medium flex items-center gap-3">
