...
const navigate = useNavigate()
  ...
  const { openJobs, openJobsPending, openJobsRefetch } = useOpenJobs(
    userInfo?.name
  );
  ...
  const { mutateAsync: handlePromote } = useMutation({
    mutationFn: async ({ id, promote }) => {
      const { data } = await axiosCommon.patch(`/job/${id}`, {
        [promote]: true,
      });
      if (!data.success) {
        toast.error("Something Went Wrong...");
  ...
  return (
    <div className="w-full px-5 py-5 font-inter border-l border-[#E4E5E8] min-h-screen">
                ...
                return (
                  <tr key={job._id} className="w-full">
                    ...
                    <td>
                      <div className="flex items-center gap-3">
                        <img
                          src="https://gist.githubusercontent.com/ShejanMahamud/040da8f1501b5097f337fe5800d2e25f/raw/c188df416d8fc24bd99a33ff391888238039cfc5/people.svg"
                          alt="people.svg"
                        />
                    ...
                    <td>
                      <div className="flex items-center gap-5 w-full">
                        <button
                          onClick={() =>
                            navigate(
                              `/dashboard/company/candidates/${job?._id}`
                            )
                          }
                        ...
                        >
                          ...
                          {isExpanded && (
                            <div className="bg-white shadow-xl border border-[#E9EAED] rounded-lg py-2 px-2 flex flex-col items-center gap-2 absolute right-0 min-w-[200px] z-50">
                              <button
                                onClick={() => handlePromoteJobModal(job)}
                              ...
                              <button
                                onClick={() => handleMakeExpire(job?._id)}
                                className="w-full py-2 hover:bg-[#E7F0FA] hover:text-primary text-[#5E6670] flex items-center gap-2 px-2"
                              >
                                <IoCloseCircleOutline className="text-xl" />
      ...
      >
        {selectedJob && (
          <form
            onSubmit={(e) => handlePromoteJob(e, selectedJob?._id)}
            className="w-full mb-5"
          >
            <h1 className="text-2xl font-medium mb-3">
              Promote Job: {selectedJob.job_title}
            </h1>
            <p className="text-[#5E6670] mb-3">
            ...
            <div className="w-full grid grid-cols-2 row-auto items-center gap-5">
              <div className="w-full bg-[#f1f2f480] rounded-lg px-5 py-5 hover:bg-[#E7F0FA]">
                <div className="bg-white px-2 py-2 rounded-lg border-[#E4E5E8] border mb-5">
                  <h1 className="font-medium text-xs mb-5">
                    ALWAYS ON THE TOP
                  </h1>
                  <div className="w-full grid grid-cols-2 row-auto items-center gap-2">
                    <img
                      src="https://gist.github.com/ShejanMahamud/bf5c8684d52c8ede1913a60c3613e91b/raw/0f0ac37b53d0e465c7a78f00642ddba33e5bc00d/featured-1.svg"
                      alt=""
                    />
                    <img
                      src="https://gist.github.com/ShejanMahamud/77a633e406245a3540026ca60be24105/raw/6b4db8004749f477f9845753e5d2fd1482953910/featured-2.svg"
                      alt=""
                    />
                    <img
                      src="https://gist.github.com/ShejanMahamud/77a633e406245a3540026ca60be24105/raw/6b4db8004749f477f9845753e5d2fd1482953910/featured-2.svg"
              ...
              <div className="w-full bg-[#f1f2f480] focus:outline-primary rounded-lg px-5 py-5 hover:bg-[#E7F0FA]">
                <div className="bg-white px-2 py-2 rounded-lg border-[#E4E5E8] border mb-5">
                  ...
                  <div className="w-full grid grid-cols-2 row-auto items-center gap-2">
                    <img
                      src="https://gist.githubusercontent.com/ShejanMahamud/0fb468dff33f87364249ab259517a83f/raw/bf7ed68e37723a2f28968f9a1c7cf5d4025f8bec/highlight.svg"
                      alt=""
                    />
                    <img
                      src="https://gist.github.com/ShejanMahamud/77a633e406245a3540026ca60be24105/raw/6b4db8004749f477f9845753e5d2fd1482953910/featured-2.svg"
            ...
            <div className="w-full flex items-center justify-between mt-10">
              <button
                ...
                className="text-[#767F8C] font-medium text-base"
              >
                Cancel
              </button>
              <button
                disabled={selectedJob?.featured || selectedJob?.highlight}
                className="bg-primary px-4 py-3 rounded-md text-white flex items-center gap-3 text-base"
              >
                <span>Promote Job</span>
                <svg
                  xmlns="http://www.w3.org/2000/svg"
