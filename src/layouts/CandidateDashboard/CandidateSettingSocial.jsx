...
const handleSocialAdd = async (e) => {
  ...
  try{
    const {data} = await axios.patch(`http://localhost:5948/user/${user?.email}`,socialInfo)
    if(data.success){
  ...
  return (
    <div className='w-full px-5 font-inter'>
        ...
        <button className='mt-5 bg-primary text-white font-medium text-lg px-4 py-2 rounded-sm w-[40%]'>Save Changes</button>
        </form>
    </div>
  )
}
